...
# Upload Dataset
df_file = st.file_uploader("Upload Dataset Anda (CSV)", type=["csv"])
if df_file is not None:
    # Load DataFrame
    df = pd.read_csv(df_file)  # Menggunakan df_file, bukan df
    st.write("### Dataset")
    st.write(df.head())
    ...
    variables = st.multiselect("Pilih Variabel untuk Clustering:", df.columns, default=['Income', 'Spending_Score', 'Last_Purchase_Amount'])
    if len(variables) >= 2:
...
if model_file is not None:
    ...
    with st.form("prediction_form"):
        ...
        if 'variables' in locals():
            for var in variables:  # Gunakan variabel yang dipilih sebelumnya
                value = st.slider(f"{var}", min_value=float(df[var].min()), max_value=float(df[var].max()), step=0.1, value=float(df[var].mean()))
                input_data.append(value)
        else:
            st.error("Variabel tidak ditemukan. Pastikan Anda memilih variabel clustering.")
